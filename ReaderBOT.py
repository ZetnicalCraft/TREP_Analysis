...
class ReaderBOT:
    ...
    def extract_data_form_start_mesa_id_to_end_mesa_id(self):
        print(f"Iniciando la extracción de datos desde la mesa: {self.start_mesa_id} a la {self.end_mesa_id - 1}")
        for mesa_id in range(self.start_mesa_id, self.end_mesa_id):  # (start, end) includes the start but not the end.
            start_time = time.perf_counter()
            print(f"Procesando mesa ID: {mesa_id}")
            ...
            print(f"Tiempo de procesamiento para Mesa ID {mesa_id}: {elapsed_time:.2f} segundos.")
    ...
    def save_to_database_by_database_name(self, archive_of_mesa_id: []):
        ...
        data_frame.to_csv(self.name_of_archive_to_save, mode='a', header=False, index=False)
